...
##########################################
# IND-IND Attention (All Shared) Mechanism
##########################################
def init_multigru_concat_ind_ind(params, nin, dim, dimctx, scale=0.01, prefix='gru_decoder_multiconcat'):
...
def multigru_concat_ind_ind(tparams, state_below,
    ...
    def _step(m_, x_, xx_,
              ...
              pctx1_, pctx2_, cc1_, cc2_, U, Wc, W_comb_att, U_att, c_att,
...
############################################
# DEP-DEP Attention (All Distinct) Mechanism
############################################
def init_multigru_concat_dep_dep(params, nin, dim, dimctx, scale=0.01, prefix='gru_decoder_multiconcat'):
    # Init with usual gru_cond function
    params = param_init_gru_cond(params, nin, dim, dimctx, scale, prefix)
...
########################################################
# DEP-IND Attention (att distinct, dec shared) Mechanism
########################################################
def init_multigru_concat_dep_ind(params, nin, dim, dimctx, scale=0.01, prefix='gru_decoder_multiconcat'):
    # Init with usual gru_cond function
    params = param_init_gru_cond(params, nin, dim, dimctx, scale, prefix)
...
def multigru_concat_dep_ind(tparams, state_below,
                            ctx1, ctx2, prefix='gru_decoder_multiconcat',
                            input_mask=None, one_step=False,
                            init_state=None, ctx1_mask=None):
    if one_step:
    ...
    def _step(m_, x_, xx_,
        ...
        ##############################################################
        # NOTE: Distinct attention for each modality, comb_att shared.
        ##############################################################
        # h1 X W_comb_att
        # W_comb_att: dim -> dimctx
        # pstate_ should be 2D as we're working with unrolled timesteps
