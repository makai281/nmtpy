...
from .fusionlayers import init_multigru_concat_dep_ind as init_gru_decoder_multiconcat
from .fusionlayers import multigru_concat_dep_ind as gru_decoder_multiconcat
...
class Model(ParentModel):
    ...
    def init_params(self):
        ...
        # convfeats (VGG: 512/ ResNet: 1024) to ctx dim (2000) for image modality
        ...
        params = get_new_layer(self.enc_type)[0](params, prefix='text_encoder'  , nin=self.embedding_dim,
                                                 dim=self.rnn_dim, scale=self.weight_init, layernorm=self.lnorm)
        params = get_new_layer(self.enc_type)[0](params, prefix='text_encoder_r', nin=self.embedding_dim,
                                                 dim=self.rnn_dim, scale=self.weight_init, layernorm=self.lnorm)
    ...
    def build(self):
        ...
        emb_enc_rnns = get_new_layer(self.enc_type)[1](self.tparams, emb_enc, prefix='text_encoder', mask=x_mask, layernorm=self.lnorm)
        ...
        emb_enc_rnns_r = get_new_layer(self.enc_type)[1](self.tparams, emb_enc_r, prefix='text_encoder_r', mask=xr_mask, layernorm=self.lnorm)
        # -> n_timesteps x n_samples x rnn_dim
    ...
    def build_sampler(self):
        ...
        proj            = get_new_layer(self.enc_type)[1](self.tparams, emb, prefix='text_encoder', layernorm=self.lnorm)

        embr            = self.tparams['Wemb_enc'][x[::-1].flatten()]
        embr            = embr.reshape([n_timesteps, n_samples, self.embedding_dim])
        projr           = get_new_layer(self.enc_type)[1](self.tparams, embr, prefix='text_encoder_r', layernorm=self.lnorm)
