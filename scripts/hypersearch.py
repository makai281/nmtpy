...
"""Single node, multi GPU grid search for nmtpy"""
...
DIMS             = [(100, 100), (100, 200), (200, 200), (200, 400), (400, 800), (620, 1000)]
...
# Use the given pairs if any
DROPOUTS          = [(0.2, 0.4, 0.4),
                     (0.3, 0.5, 0.5)]

# Learning rate
#LRATES            = [0.0004, 0.002, 0.02]

# Let's fix the learning rate
LRATES             = [0.0004]

WDECAY             = [0., 1e-5, 3e-5, 5e-5]
...
def generate_experiments():
    ...
    if DROPOUTS is not None:
        ...
        douts = itertools.product(E_DROPOUT, C_DROPOUT, O_DROPOUT)
    for seed in SEEDS:
        for dim in DIMS:
            ...
            for arch in ARCHS:
                for lrate in LRATES:
                    for wdecay in WDECAY:
                        for edrop, cdrop, odrop in douts:
                            EXPERIMENTS.append({
                                'lrate'         : lrate,
                                'decay-c'       : wdecay,
                                'model-type'    : arch,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--max-epochs', type=int, help='Max epoch for each run.', default=100)
